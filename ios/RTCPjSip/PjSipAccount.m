...
- (NSDictionary *)toJsonDictionary {
    ...
    NSString *statusText = [PjSipUtil toString:(pj_str_t *) pjsip_get_status_text(info.status)];

    // Format registration status
    NSDictionary * registration = @{
        @"status": statusText,
        @"statusDefault": [PjSipUtil toString:(pj_str_t *) pjsip_get_status_text(info.status)],
        @"statusText": [PjSipUtil toString:&info.status_text],
        @"active": ([statusText  isEqual: @"Trying"]) ? @true : ([statusText  isEqual: @"OK"]) ? @true : @false,
        @"activeDefault": @"test",
        @"reason": @"test"
    };
